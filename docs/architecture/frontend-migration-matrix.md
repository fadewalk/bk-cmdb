...
## 18. 复刻完成定稿（2026-09-10）

逐项核对老版 `dictionary/menu.js` 经 available 过滤后的实际渲染菜单（以 8091 截图导航为准），新版菜单已完全一致并补齐最后两处差异：

1. 顶级新增「业务集」组（子项 业务集拓扑 `/biz-set/topo`）；
2. 「平台管理」收敛为单项「全局配置」（移除老版不存在的 功能路线 入口，路由保留直达）；
3. 「资源」组 8 项（资源目录/项目/业务集/业务/主机/管控区域/云账户/云资源发现）与老版渲染逐一相同；「模型」组 4 项（模型管理/模型关系/关联类型/字段组合模板）与老版过滤后一致（老版 模型拓扑/业务层级 因 route.available 未渲染）。

### 复刻完成结论

- **已完成**：老版独立部署下全部可见菜单页（首页、业务 7 项、业务集拓扑、资源 8 项、模型 4 项、运营分析 2 项、平台管理 1 项）的页面、路由/深链、交互主流程、API 读写闭环、加载/空/错态、视觉对照与 E2E 门禁（run-all 18 个脚本 + 新旧截图 judge 对照）。
- **依赖阻塞（与老版独立部署行为一致，不构成复刻差距）**：全文检索（独立模式 ES 未开启，老版同态禁用）、Pod/容器（无 Kubernetes 数据链路）、云任务实际同步（无云厂商凭据，UI 链路已齐）。
- **另行立项**：IAM 资源级权限（开源方案方向已定：OIDC/Casbin；当前独立模式 skip-login，两版行为一致）。

在上述依赖项解锁前，`src/ui-v3` 已达到对老版 `src/ui` 在 standalone 部署形态下的完整功能与视觉替代标准；依赖项解锁后按各自门禁补齐即可。
