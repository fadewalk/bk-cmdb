...
> - **完整替代**：页面、流程、异常和验证均达到替代门禁；不能仅凭路由存在、主流程可用或一次 E2E 通过宣称。

> **2026-09-11 文档入口**：完整架构/安全/可靠性审计见 [`architecture-industry-benchmark-audit.md`](architecture-industry-benchmark-audit.md)；bk-cmdb 与 bk-lite 对比见 [`bk-cmdb-vs-bk-lite-cmdb.md`](bk-cmdb-vs-bk-lite-cmdb.md)；standalone 安全与可靠性门禁见 [`standalone-security-boundary.md`](standalone-security-boundary.md) 和 [`standalone-reliability-runbook.md`](standalone-reliability-runbook.md)。当前状态必须区分“页面存在、核心流程、API 写回、异常/权限、视觉、E2E、旧版全量可达、依赖阻塞”。
...
## 18. 复刻完成定稿（2026-09-10，历史口径；菜单基线已由 B35（2026-09-12）复核修订）

历史定稿曾按当时的导航截图将资源组记录为 8 项。B35 对照老版 `dictionary/menu.js` 的 `route.available` 最终可见结果并在 8090/8091 回归后，当前严格菜单口径为：业务 7 项；资源 4 项（资源目录/管控区域/云账户/云资源发现）；模型 4 项（模型管理/模型关系/关联类型/字段组合模板）；运营分析 2 项；平台管理 1 项。项目/业务集/业务/主机仍保留内部 canonical/legacy 路由和页面能力，但不作为资源同级导航项。

### 当前复刻结论

- **菜单基线已实测**：B35 断言资源 4 项、模型 4 项、业务有 bizId 时 7 项，无业务时不生成 `/business//` 伪链接，平台菜单按旧版 configAdmin 权限契约过滤；run-all 汇总入口已纳入现有批次及 B35。
- **真实实现/兼容重定向分开统计**：旧深链仍保留，但部分页面是统一工作台或抽屉形态，不再宣称全部是旧版独立页面。
- **依赖阻塞**：全文检索（独立模式 ES 未开启）、Pod/容器（无 Kubernetes 数据链路）、云任务实际同步（无云厂商凭据）。
- **另行立项**：IAM 资源级权限与正式登录（当前独立模式非 IAM 默认允许，IAM 模式走 `auth/verify`，fail-closed 菜单/路由保护已补）。

## 19. 表单形态对齐批次（2026-09-10，B22）

用户截图对照发现四处老版 sideslider/交互形态差异，逐项修复并经 judge 复审通过：

1. **新建/编辑业务**：居中弹窗 → 老版 800px 右侧滑出面板「创建 业务/编辑 业务」，「属性」tab + 分组两列表单（基础信息：业务名称/生命周期/时区/语言/描述；角色：运维/产品/测试/开发/操作人员），按钮 提交/取消；新增 操作人员 字段。
2. **新建/编辑云账户**：弹窗 → 右侧滑出面板「新建账户/编辑账户」，字段对齐老版（账户名称/账户类型/ID/Key+连通测试/备注），接 `/cloud/account/verify` 连通测试；按钮 提交/取消；编辑态厂商锁定、密钥留空不修改。
3. **主机列表页**：头部改为老版「← 主机 ↗」（返回+新窗口打开，隐藏全局面包屑避免重复）；默认 tab 未分配且 scope 写入 query；目录树改为 主机池(计数)→空闲机(计数)+自定义目录，计数恒显示（0 也显示），默认选中空闲机并写 query.directory；导入改为老版 import-file 两步抽屉（上传文件→选择关联模型，op:1 解析/op:2 提交，20M xlsx+下载模板+拖拽上传），移除 CSV 粘贴路径。
4. **分页控件**：业务/项目/业务集列表的「每页 20 条」固定文本统一改为 老版每页条数下拉（10/20/50/100）。
5. **菜单回退**：老版 8091 实际导航无顶级「业务集」组（route.available 过滤），撤销 17 节中的该项，菜单以老版渲染态为准。

验收：run.cjs 导入段改断言两步抽屉结构；b15 云账户编辑改断言抽屉+提交；全量 run-all 18 脚本通过；四处新旧截图 judge 复审全部 pass（bizCreate/accountCreate/hostList/hostImport）。
