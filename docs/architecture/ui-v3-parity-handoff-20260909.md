...
- 像素级对照（commit a2813fc3a0）：旧前端已可在本地真实运行——`cd src/ui && npm run build && PORT=8091 node builder/serve.js`（API 代理 8090、注入 admin/internal 登录态），与新前端同屏截图对照。该批次：新建服务模板复刻旧版整页（三段折叠表单、走 `create/proc/service_template/all_info` 一次创建模板+进程+属性、无进程确认、创建成功弹窗）；服务分类按扁平契约（`bk_parent_id`）解析两级，默认 Default(#1)/(#2)；进程数量接 `count_info`；全局表格白底表头+外边框+12px、tips 蓝条(ⓘ+关闭)、业务选择器 `name (id)`、Logo 蓝鲸配置平台、空态"暂无数据+立即创建"入表格内。
