<template>
  <div class="page-card biz-detail" v-loading="loading">
    <div class="crumb-row">
      <span class="back-btn" @click="goBack">‹ 业务集</span>
      <span class="crumb-title">业务集详情【{{ detail?.bk_biz_set_name || id }}】</span>
    </div>
    ...
    <template v-if="tab === 'property'">
      ...
      <el-empty v-else-if="!loading" description="业务集不存在" />
    </template>
...
const id = computed(() => Number(route.params.bizSetId) || null)
...
function goBack() { router.push('/resource/biz-set') }
...
async function loadDetail() {
  ...
  try {
    ...
    detail.value = ((data?.info || []).find((b) => b.bk_biz_set_id === id.value)) || null
...
async function loadAudit() {
  ...
  try {
    const data = await searchInstAudit({
      condition: { bk_obj_id: 'biz_set', resource_type: 'biz_set', resource_id: id.value },
