...
;(async () => {
  ...
  try {
    ...
    ok('业务同步页加载')
    const moduleGroups = await page.locator('.biz-sync-page .toolbar .el-select').count()
    if (moduleGroups >= 2) ok('业务同步筛选(业务/服务模板)就绪,分组在选模板后加载(B26 覆盖)')
    else fail('业务同步差异', '筛选器缺失')
