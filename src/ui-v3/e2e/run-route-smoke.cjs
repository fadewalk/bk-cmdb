...
;(async () => {
  ...
  try {
    ...
    await openHash(page, `#/business/${defaultBizId}/index`)
    ...
    await openHash(page, `#/business/${defaultBizId}/host/1`)
    ...
    await openHash(page, `#/business/${defaultBizId}/service/template/create`)
    assert(!routePath(page).startsWith('/404'), `服务模板创建深链落到 404: ${routePath(page)}`)
    await openHash(page, `#/business/${defaultBizId}/service/template/details/1`)
    assert(!routePath(page).startsWith('/404'), `服务模板详情深链落到 404: ${routePath(page)}`)
    await openHash(page, `#/business/${defaultBizId}/service/operational/template/1`, `#/business/${defaultBizId}/service/template/details/1`)
    ...
    await openHash(page, `#/business/${defaultBizId}/host-apply/template`)
    assert(!routePath(page).startsWith('/404') && routePath(page).includes('mode=template'), `旧版 host-apply 深链未兼容: ${routePath(page)}`)
    await openHash(page, `#/business/${defaultBizId}/set/sync/5`, `#/business/${defaultBizId}/set/template?action=sync&templateId=5`)
    assert(routePath(page).startsWith(`/business/${defaultBizId}/set/template`), `旧版 set-sync 深链未兼容: ${routePath(page)}`)
    await openHash(page, `#/business/${defaultBizId}/synchronous/module/7/12,13`, `#/business/sync?template=7&modules=12,13&biz=${defaultBizId}&source=module`)
