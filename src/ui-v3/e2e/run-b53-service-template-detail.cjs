...
async function main() {
  ...
  await page.route('**/api/v3/find/proc/service_template/all_info', (route) => routeJson(route, envelope(true, { id: 72, bk_biz_id: 99, name: 'b53-tpl', service_category_id: 501, attributes: [], processes: [] })))
  ...
  try {
    ...
    const detail = page.locator('.service-template-details')
    await detail.waitFor({ state: 'visible', timeout: 10000 })
    const detailText = await detail.textContent()
    assert(detailText.includes('配置'), '详情页配置 tab 未加载')
