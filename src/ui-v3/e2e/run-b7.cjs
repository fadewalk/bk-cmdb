...
;(async () => {
  ...
  try {
    ...
    // 高级筛选漏斗按钮(旧版契约:直接打开 400px 高级筛选侧滑)
    const filterBtn = page.locator('.option-filter')
    if (await filterBtn.isVisible()) ok('高级筛选漏斗按钮可见')
    await filterBtn.click()
    await page.waitForTimeout(600)
    const filterPanel = await page.locator('.advanced-host-filter').isVisible().catch(() => false)
    if (filterPanel) ok('高级筛选侧滑打开')
    else fail('高级筛选侧滑', '未打开')
    const ipLabel = await page.locator('.advanced-host-filter').getByText('内网IP', { exact: true }).count()
    if (ipLabel > 0) ok('侧滑 IP 选项渲染')
