...
|---|---|---|---|
| #/index | 首页(搜索+地图) | ✅ | — |
| #/business/{id}/index | 业务拓扑(左树+主机/服务实例/节点信息) | 🔨 | 树节点右键菜单、主机分页、字段显示设置 |
| #/business/{id}/service/instance | 服务实例(老版并入拓扑页) | 🔨 | 新建向导(选模块→选主机→加进程)、进程编辑 |
| #/business/{id}/service/template | 服务模板 | 🔨 | 新建/编辑/克隆/删除真实交互(本批) |
| #/business/{id}/set/template | 集群模板 | ⬜ | 新建/编辑/删除(仿服务模板) |
| #/business/{id}/service/cagetory | 服务分类 | 🔨 | 新建/编辑/删除分类(本批) |
| #/business/{id}/host-apply | 主机自动应用 | ⬜ | 规则创建/编辑向导 |
| #/business/{id}/custom-query | 动态分组 | 🔨 | 新建分组(名称+条件,本批简化版) |
...
- **B1(本批)**:服务模板真实交互(新建/编辑/克隆/删除)+ 服务分类增删改 + 动态分组新建
- **B2**:集群模板增删改 + 操作审计完整筛选 + 主机自动应用启用流程
- **B3**:业务拓扑细节(右键菜单/分页/字段设置)+ 服务实例向导
